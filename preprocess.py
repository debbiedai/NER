...
def split_fold(all_file_list, n_fold, save_path):
    ...
    for i in range(n_fold):
        print("fold:", i)
        print('test_id:', all_data[test_ids[i]])
...
def preprocess(fold_csv_path, save_path):
    for j in range(10):
        ...
        for file in fold_data['file_name']:
            ...
            if not os.path.isdir('./plaintext'):
                os.mkdir('./plaintext')
            with open('./plaintext/' + file.replace('.xml', '.txt'), 'w') as f:
                f.write(text)
                f.close()
            with open('./plaintext/' + file.replace('.xml', '_chr_annotations.txt'), 'w') as f:
                for anno in annotations:
                    offset, length, label, _ = anno
                    f.write(str(offset) + ' ' + str(length) + ' ' + str(label) + '\n')
                f.close()

            with open('./plaintext/' + file.replace('.xml', '_token_annotations.txt'), 'w') as f:
                for ans in token_index:
                    offset, length, label = ans
                    f.write(str(offset) + ' ' + str(length) + ' ' + str(label) + '\n')
                f.close()
